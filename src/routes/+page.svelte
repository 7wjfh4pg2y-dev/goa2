<script lang="ts">
	...
	// --- navigation ---
	// a cinematic, MOBA/trailer-style "enter" hit (synthesized — no audio asset):
	// whoosh riser → deep boom + a detuned brass "braaam", all drenched in reverb
	function playEnterSfx() {
		try {
			...
			comp.threshold.value = -14; comp.knee.value = 12; comp.ratio.value = 14; comp.attack.value = 0.003; comp.release.value = 0.25;
			comp.connect(ac.destination);
			const dry = ac.createGain(); dry.gain.value = 0.95; dry.connect(comp);

			// convolver reverb (synth impulse) on a parallel wet send — this is what
			// makes it sound "big" rather than a cheap beep
			...
			const wet = ac.createGain(); wet.gain.value = 0.34; verb.connect(wet); wet.connect(comp);
			const send = (node: AudioNode) => { node.connect(dry); node.connect(verb); };
			...
			// 1) whoosh riser building into the hit
			const riser = ac.createBufferSource(); riser.buffer = noise;
			const rf = ac.createBiquadFilter(); rf.type = 'bandpass'; rf.Q.value = 0.7;
			rf.frequency.setValueAtTime(260, t); rf.frequency.exponentialRampToValueAtTime(6000, hit);
			const rg = ac.createGain();
			rg.gain.setValueAtTime(0.0001, t); rg.gain.exponentialRampToValueAtTime(0.45, hit - 0.02); rg.gain.exponentialRampToValueAtTime(0.0001, hit + 0.14);
			riser.connect(rf); rf.connect(rg); send(rg);
			riser.start(t); riser.stop(hit + 0.2);

			// 2) the boom: sub-bass drop + punchy noise transient
			const sub = ac.createOscillator(); sub.type = 'sine';
			sub.frequency.setValueAtTime(170, hit); sub.frequency.exponentialRampToValueAtTime(40, hit + 0.55);
			const sg = ac.createGain();
			sg.gain.setValueAtTime(0.0001, hit); sg.gain.exponentialRampToValueAtTime(1, hit + 0.015); sg.gain.exponentialRampToValueAtTime(0.0001, hit + 1.2);
			sub.connect(sg); send(sg); sub.start(hit); sub.stop(hit + 1.3);

			const burst = ac.createBufferSource(); burst.buffer = noise;
			const bf = ac.createBiquadFilter(); bf.type = 'lowpass'; bf.frequency.value = 2200;
			const bng = ac.createGain(); bng.gain.setValueAtTime(0.7, hit); bng.gain.exponentialRampToValueAtTime(0.0001, hit + 0.16);
			burst.connect(bf); bf.connect(bng); send(bng); burst.start(hit); burst.stop(hit + 0.22);

			// 3) the "braaam" — a stack of detuned sawtooths through an opening
			// low-pass; the classic epic-trailer horn
			const braamGain = ac.createGain();
			braamGain.gain.setValueAtTime(0.0001, hit);
			braamGain.gain.exponentialRampToValueAtTime(0.5, hit + 0.12);
			braamGain.gain.setValueAtTime(0.5, hit + 0.9);
			braamGain.gain.exponentialRampToValueAtTime(0.0001, hit + 1.6);
			const braamLp = ac.createBiquadFilter(); braamLp.type = 'lowpass'; braamLp.Q.value = 0.9;
			braamLp.frequency.setValueAtTime(180, hit); braamLp.frequency.exponentialRampToValueAtTime(1400, hit + 0.5);
			braamLp.connect(braamGain); send(braamGain);
			for (const cents of [-9, -4, 0, 5, 11]) {
				const o = ac.createOscillator(); o.type = 'sawtooth';
				o.frequency.value = 55; o.detune.value = cents; // ~A1, richly detuned
				o.connect(braamLp); o.start(hit); o.stop(hit + 1.7);
			}
			const oct = ac.createOscillator(); oct.type = 'sawtooth'; oct.frequency.value = 110; oct.detune.value = 4;
			oct.connect(braamLp); oct.start(hit); oct.stop(hit + 1.7);

			// 4) metallic shwing
			for (const f of [1245, 1870]) {
				const o = ac.createOscillator(); o.type = 'sawtooth';
				o.frequency.setValueAtTime(f * 1.5, hit); o.frequency.exponentialRampToValueAtTime(f, hit + 0.12);
				const hf = ac.createBiquadFilter(); hf.type = 'highpass'; hf.frequency.value = 900;
				const g = ac.createGain(); g.gain.setValueAtTime(0.12, hit); g.gain.exponentialRampToValueAtTime(0.0001, hit + 0.4);
				o.connect(hf); hf.connect(g); send(g); o.start(hit); o.stop(hit + 0.45);
			}
