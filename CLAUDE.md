...
- **Card effects (`src/lib/effects.ts`, tested):** `$ms.effects` — lingering effects switched on from a played card's zoom view (owner or host), duration pre-picked from the card text (`detectDuration`: This turn / Next turn / This round). The card glows in the player's colour with a ⏳ badge (dash, rows, boards), rows get chips, the left HUD lists them (`cardLayer.showCard`). Expiry runs in `applyCardReq` advance (`expireEffects`); the host logs endings. "Next turn" on turn 4 is capped at round end.
- **Phone layout (≤760px wide ONLY — desktop/iPad untouched):** GameView `mobile` (bind:innerWidth) swaps the left HUD for a top bar (☰ · R/T · tie · waves · split life `images/life_split.png` · gold ±) + ☰ drawer (spawn O/B×M R H, effects, activity, 3×2 view grid + saved views, lobby/leave) + waves/life sheet; the board sits in `.boardarea.mob` between strip and dash. CardLayer `mobile` swaps the right panel for `.mstrip` (scrollable player cards: portrait, name, hero, Lv, ⏱, current card, stats w/ item pips) and the dash for `.mdash` (you + stats/pips · I–IV · discard · deck · 2×3 buttons: ⏱ ◎ / hide fan / tokens undo(host, red)) with a floating `.mact`. Shared controls are snippets: `radiusCtl`, `tokenCtl`, `actionBody` (Take back / Activate effect? / Next turn). Detailed stat art: `images/stats/*.png`. Overlays get `@media (max-width:760px)` sizing.
