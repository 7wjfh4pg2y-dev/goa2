...
- **Tokens (`src/lib/tokens.ts`, tested):** per-hero kit `HERO_KIT` (no kit → shelf button disabled, ◈). Shelf pick → `onArmToken` → GameView `pendingToken` (held-token cursor, like minion spawn) → tap hex → `placeToken`. Rules: Min's Blast/Dud = mines placed `faceDown` (skull back in owner colour; owner sees a B/D peek), flip from the piece toolbar; markers are one-of-each; poison/bounty dropped on an enemy hero get `attachedTo` = that hero (drawn as a badge, rides with them) and the HUD status is DERIVED from that (`statusFrom`); moving it off / deleting ends it. Round end (`applyCardReq` advance) sweeps tokens except `keepsThroughRound` (trees, companions, Snorri runes).
- **Holding pieces (BoardCanvas):** drag or tap-to-pick-up → the original stays (dimmed `.lifted`), a `.ghost` copy (same `pieceBody` snippet) rides the cursor, the hex under it is filled in the viewer's colour (`holdColor`), cursor = grabbing. Drops only land on real hexes (`hexAt`); off-board = back where it was. Spawns / shelf tokens use the same ghost via `placeGhost`; off-board tap → `onCancelPlace`. (No `<slot>` in BoardCanvas — snippets and slots can't mix.)
- **Area radius:** dash ◎ button (next to the gold initiative badge) sets `$ms.radii[pid]` (1–8); BoardCanvas `areas` draws hex-distance fill + outer edge in the player's colour around their hero; cleared on every turn advance (`applyCardReq`).
- **Card effects (`src/lib/effects.ts`, tested):** `$ms.effects` — lingering effects switched on from a played card's zoom view (owner or host), duration pre-picked from the card text (`detectDuration`: This turn / Next turn / This round). The card just glows (pulsing) in the player's colour everywhere (`.fxwrap.fx`, a `filter: drop-shadow` on the wrapper — TurnSlot's flip faces are `overflow:hidden`, so a canvas box-shadow gets clipped) — no badges/icons. "Active abilities" (`abilityRows` in GameView: one fixed row per hero, 5 card-colour pips GOLD/SILVER/RED/BLUE/GREEN, timing) is in the desktop left HUD and the phone ☰ menu; tap a live row → `cardLayer.showCard`. Expiry runs in `applyCardReq` advance (`expireEffects`); the host logs endings. "Next turn" on turn 4 is capped at round end.
- **Reveal curtain:** rows by team (your team on top; ≤2 players share one row); `fitReveal` picks the column count per team row that maximises card size for the window; title never wraps.
- **Phone layout (≤760px wide ONLY — desktop/iPad untouched):** GameView `mobile` (bind:innerWidth) swaps the left HUD for a top bar (☰ · R/T · tie · waves · split life `images/life_split.png` · ULT (centred between life and gold; always previewable, purple pulse once unlocked) · gold ±; measured to fit 360px) + ☰ drawer (spawn O/B×M R H, effects, activity, 3×2 view grid + saved views, lobby/leave) + waves/life sheet (drag down to close); the board sits in `.boardarea.mob` between strip and dash. CardLayer `mobile` swaps the right panel for `.mstrip` (scrollable player cards: portrait, name, hero, Lv, ⏱, current card, stats w/ item pips) and the dash for `.mdash` (you + stats/pips · I–IV · discard · deck · 2×3 buttons: ⏱ ◎ / hide fan / tokens undo(host, red)) with a floating `.mact`. Shared controls are snippets: `radiusCtl`, `tokenCtl`, `actionBody` (Take back / Activate effect? / Next turn). Detailed stat art: `images/stats/*.png`. Overlays get `@media (max-width:760px)` sizing. Deck view on phones (`.deckmodal.mob`): full-screen, tabs Hand/Deck/Upgrade/Removed (`deckTab`), tap = select (ult/basics tap = enlarge), sticky action bar with 🔍 Enlarge.
